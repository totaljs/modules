...
var REG_LINK_1 = /\<.*?\>+/g;
var REG_LINK_2 = /(!)?\[[^\]]+\][\:\s\(]+.*?[^)\s$]+/g;
var REG_LINK_3 = /(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig;
var REG_LINK_4 = /(^|[^\/])(www\.[\S]+(\b|$))/gim;
var REG_IMAGE = /(\[)?\!\[[^\]]+\][\:\s\(]+.*?[^\s$]+/g;
var REG_FORMAT = /\*{1,2}.*?\*{1,2}|_{1,3}.*?_{1,3}/g;
var REG_KEYWORD = /(\[.*?\]|\{.*?\})/g;
...
Markdown.prototype.parseList = function(line) {
    ...
    self.current.push({ type: first, value: self.parseOther(line.substring(2)) });
    return true;
};
...
Markdown.prototype.parseKeyword = function(text) {
    ...
    for (var i = 0; i < length; i++) {
        ...
        text = text.replace(o, self.getReplace(o, self.onKeyword(type, value) || ''));
    }
